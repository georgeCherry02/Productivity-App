...
export class BarChart extends Component {
    render() {
        var positive_data = {"x": this.props.data.x, "y": []};
        var negative_data = {"x": this.props.data.x, "y": []};
        for (var i = 0; i < this.props.data.y.length; i++) {
            if (this.props.data.y[i] < 0) {
                negative_data.y.push(Math.abs(this.props.data.y[i]));
                positive_data.y.push(0);
            } else {
                negative_data.y.push(0);
                positive_data.y.push(this.props.data.y[i]);
            }
        }

        return (
            <TempChart positive_data={positive_data}
...
// Need to split up positive and negative data here
class TempChart extends Component {
    render() {
        const data = this.props.positive_data;
        var axis_increment_value, axis_labels_amount, max_y_axis_label;
        var x_axis_labels = [], y_axis_labels = [];
        var max_value = Math.max(...data.y);
        ...
        for (i = 0; i < data.x.length; i++) {
            x_axis_labels.push(<HistogramAxisLabel value={data.x[i]} key={i}/>);
        }
...
class HistogramAxisLabel extends Component {
    render() {
        ...
        // This AGHHH is because of varying amounts of text need to be scaled appropriately
        // Should be careful
        return(
